...
{
    Class с = objc_getClass("NSBundle");
    if (с) {
        method_exchangeImplementations(class_getInstanceMethod(с, @selector(bundleIdentifier)),
                                       class_getInstanceMethod(с, @selector(__bundleIdentifier)));
        return YES;
    }
	return NO;
}
